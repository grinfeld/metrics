...
import com.mikerusoft.metrics.annotations.MetricParam;
import com.mikerusoft.metrics.annotations.Param;
import com.mikerusoft.metrics.aop.Proceeder;
import com.mikerusoft.metrics.reflections.ReflectionUtils;
...
import static com.mikerusoft.metrics.MethodCacheStore.INVALID;
...
public class MetricIntercepter {
    ...
    private final MethodCacheStore cache;
    ...
    public Object invoke(Object object, Method method, Object[] arguments, Proceeder proceeder) throws Throwable {
        ...
        try {
            ...
            try {
                ...
                log.trace("Error in recording metrics {}", ex.getMessage());
            }
            return result;
        } catch (Exception e) {
            try {
    ...
    private String updateMetricNameFromMethodArgs(Method method, Object[] arguments, AddMetric annotation, String name) {
        if (annotation.withParams().length > 0) {
            try {
                ...
                if (!Utils.isEmpty(resolvedParams)) {
                    name = name + "." + resolvedParams;
                }
            } catch (Exception ex) {
                log.trace("Error in resolving metric params {}", ex.getMessage());
            }
        }
        return name;
    }
